...
class SolarLite_Mail_Message
{
    ...
    public function __construct()
    {
        ...
        if (!$this->_config['transport']) {
            $config = SolarLite_Config::get('mail', array());
            if (isset($config['transport'])) {
                $class = 'SolarLite_Mail_Transport_Adapter_' . ucfirst($config['transport']);
            } else {
                $class = 'SolarLite_Mail_Transport_Adapter';
            }
            $this->_transport = new $class($config);
        }
    }
    ...
    {
        // create the part
        $part = new SolarLite_Mail_Message_Part();
    ...
    public function addHeader($label, $value, $replace = true)
    {
        ...
        $label = SolarLite_Mime::headerLabel($label);
    ...
    public function fetchHeaders()
    {
        ...
        foreach ($headers as $key => $val) {
            ...
            $val[0] = SolarLite_Mime::headerLabel($val[0]);
            $headers[$key][1] = SolarLite_Mime::headerValue(
                $val[0],
                $val[1],
                $this->_charset,
                $this->_crlf
    ...
    public function fetchContent()
    {
        ...
        if ($this->_text && $this->_html && $this->_atch) {
            ...
            $alt = new SolarLite_Mail_Message_Part();
